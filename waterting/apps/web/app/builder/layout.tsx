...
export default function BuilderLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { label: 'STRATEGIC_OVERVIEW', href: '/builder/dashboard', icon: BarChart3 },
    { label: 'DEVELOPMENT_PROJECTS', href: '/builder/projects', icon: Pickaxe },
    { label: 'ASSET_INVENTORY', href: '/builder/inventory', icon: Building2 },
    { label: 'STAKEHOLDER_REGISTRY', href: '/builder/leads', icon: Users2 },
    { label: 'EXECUTION_BOOKINGS', href: '/builder/bookings', icon: FileSignature },
    { label: 'MARKET_OUTREACH', href: '/builder/ads', icon: Megaphone },
    { label: 'COLLABORATION_TEAM', href: '/builder/team', icon: UserCheck },
  ];
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 font-sans selection:bg-primary selection:text-white">
          ...
          {!collapsed && (
            <div className="flex items-center gap-3 animate-in fade-in duration-700">
               ...
               <div>
                  ...
                  <span className="text-[8px] font-black text-primary uppercase tracking-[0.3em] mt-1 block">DEVELOPER_OPS</span>
               </div>
            </div>
          )}
          {collapsed && (
          ...
          >
             {collapsed ? <ChevronRight size={18} /> : (
               <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest">
                  <ChevronLeft size={14} className="group-hover:-translate-x-1 transition-transform" /> COLLAPSE_OPS
               </div>
             )}
          </button>
          ...
          <div className={`flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/5 group hover:bg-rose-500/10 hover:border-rose-500/20 transition-all ${collapsed ? 'justify-center' : ''}`}>
            ...
            {!collapsed && (
              <div className="flex-1 min-w-0">
                <div className="text-[10px] font-black text-white uppercase tracking-widest truncate">PROJECT_ROOT</div>
                <button 
      ...
      <main className={`flex-1 min-h-screen transition-all duration-500 ${collapsed ? 'ml-24' : 'ml-72'} p-12 bg-white relative`}>
         <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-primary via-emerald-500 to-transparent opacity-30" />
