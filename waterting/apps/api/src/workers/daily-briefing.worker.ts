    ...
    try {
      ...
      for (const tenant of tenants) {
        try {
          ...
          this.logger.error(`Briefing failed for ${tenant.name}`, tenantErr);
          Sentry.captureException(tenantErr);
        }
      }
      this.logger.log(`Worker DailyBriefingWorker completed: ${processed} tenants processed`);
    } catch (err) {
      this.logger.error('Daily briefing cron failed entirely:', err);
