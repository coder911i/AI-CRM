...
let PaymentsController = class PaymentsController {
    ...
    create(user, bookingId, data) {
        return this.paymentsService.create(user, bookingId, data);
    }
    findAll(user, bookingId) {
        return this.paymentsService.findAllByBooking(user, bookingId);
    }
    recordPayment(user, id, data) {
        return this.paymentsService.recordPayment(user, id, data);
    }
    async exportPayments(user, bookingId, res) {
        const payments = await this.prisma.payment.findMany({
            where: { bookingId },
            orderBy: { dueDate: 'asc' }
        });
        const header = 'Description,Due Date,Amount,Status,Paid At,Method,UTR\n';
        const rows = payments.map((p) => `"${p.description}","${p.dueDate.toLocaleDateString('en-IN')}",${p.amount},"${p.paidAt ? 'PAID' : 'DUE'}","${p.paidAt?.toLocaleDateString('en-IN') || ''}","${p.method || ''}","${p.utr || ''}"`).join('\n');
...
exports.PaymentsController = PaymentsController = __decorate([
    (0, common_1.Controller)('bookings/:bookingId/payments'),
